...
BEGIN
    ...
    SELECT e.emp_id                         AS Employee_Number
         ...
         , p.user_text_1                    AS Position_Title
         ...
         , CASE
               ...
               ELSE jt.title
           END                              AS Original_Job_Position_Title,
         , es.emp_status_code               AS Employee_Status
    FROM DBShrpn.dbo.employee e
    INNER JOIN DBShrpn.dbo.individual i ON
    ...
    LEFT JOIN DBShrpn.dbo.pos_title pt ON
        ...
        pt.eff_date = (
                        SELECT MAX(eff_date)
                        FROM DBShrpn.dbo.pos_title t
                        WHERE t.pos_id = pt.pos_id
                        AND t.eff_date <= GETDATE()
                    )
    LEFT JOIN DBShrpn.dbo.job_title jt ON
        ...
        jt.eff_date = (
                        SELECT MAX(eff_date)
                        FROM DBShrpn.dbo.job_title t
                        WHERE t.job_id = jt.job_id
                            AND t.eff_date <= GETDATE()
                    )
    ),
    cteT2 (
