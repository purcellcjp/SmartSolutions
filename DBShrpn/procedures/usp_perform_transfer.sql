...
BEGIN
    ...
    DECLARE @rehire_override			bit    --CHAR(01)
...
*/
            ...
            SELECT @emp_status_code	=  s.emp_status_code
            FROM DBShrpn.dbo.emp_status s
            WHERE s.emp_id = @emp_id_01
              AND s.status_change_date = (
                                          SELECT MAX(t.status_change_date)
            ...
            SELECT	@old_entity					=	'',
                    ...
                    @new_curr_code				=	'',
                    @v_eff_date_dt              = CAST(@eff_date_01 AS datetime)

            SET @v_cal_year = YEAR(@v_eff_date_dt)


            --	Find the old entity & old tax entity & eff date
            SELECT @old_entity = empl_id
                 , @old_tax_entity = tax_entity_id
                 , @eff_date = eff_date
            FROM DBShrpn.dbo.uvu_emp_employment_most_rec
            WHERE (emp_id = @emp_id_01)
            ...
            /* Removing date specific logic - moving lookups to main
            ...
            */
            ...
            SELECT	@ea_emp_id					= ea.emp_id,
                    @ea_assigned_to_code		= ea.assigned_to_code,
                    @ea_job_or_pos_id			= ea.job_or_pos_id,
                    @ea_eff_date				= ea.eff_date,
                    @ea_next_eff_date			= ea.next_eff_date,
                    @ea_prior_eff_date			= ea.prior_eff_date,
                    ...
                    @i_hourly_rate_amt			= ea.hourly_pay_rate,
                    @i_period_amt				= ea.pd_salary_amt,
                    @i_work_tm_code				= ea.work_tm_code,

                    @assignment_end_date        = ea.end_date,
                    @job_position_end_date		= ea.end_date,
                    @assigned_to_code			= ea.assigned_to_code
            ...
            EXECUTE DBShrpn.dbo.usp_upd_hrpn_02_trn
                  @p_emp_id                         = @emp_id_01
                , @p_empl_id                        = @old_entity
                , @p_new_empl_id                    = @empl_id_01
                , @p_transfer_date                  = @v_eff_date_dt		--CAST(@eff_date_01 AS datetime)
                , @p_assign_to                      = @assigned_to_code
                , @p_job_or_pos_id                  = '99999'							-- Default Position
                , @p_org_grp_id                     = @organization_group_id_01		--CAST(@organization_group_id_01 AS INT)
                ...
                , @p_eff_date                       = @v_eff_date_dt								--	emp_employment
            ...
            FROM	DBShrpn.dbo.uvu_emp_assignment_most_rec
            WHERE	emp_id = @emp_id_01
